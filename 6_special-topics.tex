...
Put 
$\prescript{2}{}{\typeB_2}(2^{2n+1})=\{g\in G(\dF_{2^{2n+1}}):f\circ \sigma(g)=g\}$. 

Also in characteristic $2$, the ``Dynkin diagram'' for $\typeF_4$ is 
$\bullet-\bullet=\bullet-\bullet$. 

In characteristic $3$, $\typeG_3$ is [dot three lines dot], so the same thing 
works. 

Note: $\#\typeE_8(2) \approx 3.3\cdot 10^{74}$. In contrast, the Monster 
