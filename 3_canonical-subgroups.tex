...
By [cite source], if $G$ is of multiplicative type, then there exists a finite 
\emph{separable} extension $K\supset k$ such that $G_K$ is diagonalizable. 
...
The group 
\[
  G=\left\{\begin{pmatrix} a & -b \\ b & a \end{pmatrix}\right\}\subset \GL(2) ,
\]
is a torus, but is not diagonalizable over the base field $\dR$. This follows 
from the fact that $\begin{pmatrix} & -1 \\ 1 \end{pmatrix}$ is not 
diagonalizable over $\dR$. But the group $G_\dC$ is a split torus, because 
there exists $p\in \GL_2(\dC)$ such that 
\[
  p\begin{pmatrix} & -1 \\ 1 \end{pmatrix} p^{-1} = \begin{pmatrix} i \\ & -i \end{pmatrix} .
\]
It follows that 
\[
  p G p^{-1} = \left\{\begin{pmatrix} a + b i \\ & a-b i \end{pmatrix}\right\}\subset \GL(2)_{/\dC} .
\]
If we had started with $G_{/\dQ}$, we would need to base-change to 
$\dQ(i)$ for $G$ to split. 
\end{example}
...
multiplicative type). For general groups $G$, we define 
\[
  \characters^\ast(G) = \hom(G_{k^\mathrm{sep}},\Gm) .
\]
[interlude: $\characters^\ast(G)(A)=\hom_{\groups_{/A}}(G_{/A},{\Gm}_{/A})$.]

\begin{example}
Let $G$ be the group in \autoref{eg:nonsplit-torus}. Then over $\dR$, all 
characters are powers of 
\[
  \begin{pmatrix} a & -b \\ b & a \end{pmatrix}\mapsto a^2 + b^2 .
\]
But over $\dC$, there are two characters $\chi_1,\chi_2$, namely those that map 
the matrix $\begin{pmatrix} a & -b\\ b & a \end{pmatrix}$ to 
$a+i b$, resp.~$a-b i$. The group $\characters^\ast(G_\dC)$ has some extra 
structure, namely an action of the group $\galois(\dC/\dR)=\langle c\rangle$. 
We have $c(\chi_1) = \chi_2$, $c(\chi_2)=\chi_1$. In general, 
$c(\chi_1^{n_1} \chi_2^{n_2}) = \chi_1^{n_2}\chi_2^{n_1}$. So under the obvious 
...
\[
  H = \left\{\begin{pmatrix} a & -b \\ b & a \end{pmatrix}:a^2+b^2=1\right\}\subset \GL(2) .
\]
The group $H$ fits inside a short exact sequence 
$1\to H\to G\xrightarrow{\det} \Gm\to 1$, so on the level of characters, we 
have a short exact sequence of $\dZ[\galois(\dC/\dR)]$-modules 
\[\
  0 \to \characters^\ast(\Gm) \to \characters^\ast(G) \to \characters^\ast(H) \to 0.
\]
Complex conjugation $c$ acts on the quotient 
$\characters^\ast(H)$ as multiplication by $-1$. 
\end{example}
...
Here, we say that $\Gamma$ acts continuously on a discrete set $X$ if the 
map $\Gamma\times X\to X$ is continuous. Equivalently, for each $x\in X$, the 
stabilizer $\stabilizer_\Gamma(x)$ is open. We need to describe the 
$\Gamma$-action on $\characters^\ast(G)$. If $\gamma\in \Gamma$ and 
$\chi\in \characters^\ast(G)$, then $\gamma(\chi):G_{k^\mathrm{sep}}\to \Gm$ 
is given by $(\gamma\chi)(g)=\gamma(\chi(\gamma^{-1} g))$ on 
$k^\mathrm{sep}$-points. 

[Maybe put interlude on $\characters^\ast(G)$ as \'etale sheaf here.]

\begin{example}
Let $K/k$ be a finite field extension. Then the Weil restriction 
$\weil_{K/k}\Gm:\algebras k\to \groups$ is defined by 
$A\mapsto (A\otimes_k K)^\times$. This is an algebraic group (a torus, in 
fact) over $k$, which satisfies $(\weil_{K/k}\Gm)_K\simeq \Gm^{[K:k]}$. 
\end{example}

\begin{example}
Let $k$ be a field of characteristic $\ne 2$. The Kummer theory tells us that 
quadratic extensions $K/k$ are all of the form $k(\sqrt d)/k$ for 
$d\in k^\times/2$. The Weil restriction 
$\weil_{k(\sqrt d)/k}\Gm$ can be identified with the subgroup 
\[
  G_d = \left\{\begin{pmatrix} a & b d \\ b & a \end{pmatrix}\right\} \subset \GL(2)_{/k} .
\]
The isomorphism type of $G_d$ depends only on the class of $d$ in 
$k^\times/2$. Moreover, $\characters^\ast(G_d)\simeq \dZ^2$, with the action 
of $\Gamma_k$ factoring through $\galois(k(\sqrt d)/k)$ and flipping 
coordinates. 
\end{example}
...
$\weil_{\dC/\dR}\Gm$, and copies of 
$\ker\left(\weil_{\dC/\dR}\Gm\xrightarrow{\det}\Gm\right)$. 
\end{example}
