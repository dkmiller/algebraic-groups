...
Consider 
\[
  \fg=\fb_3 = \begin{pmatrix} \ast & \ast & \ast \\ & \ast & \ast \\ & & \ast \end{pmatrix}\subset \matrices_3 = \gl_3 .
\]
We claim that $\fb_3$ is solvable. Note that 
\begin{align*}
\begin{pmatrix} \ast & \ast & \ast \\ & \ast & \ast \\ & & \ast \end{pmatrix} / \begin{pmatrix} & \ast & \ast \\ & & \ast \\ & & \end{pmatrix} &\simeq \gl_1\times \gl_1\times \gl_1 \\
\begin{pmatrix} & \ast & \ast \\ & & \ast \\ & & \end{pmatrix} / \begin{pmatrix} & & \ast \\ & \\ & \end{pmatrix} &\simeq \gl_1\times \gl_1 
\end{align*}
so $\fg$ is solvable. In general, the Lie algebra $\fb_n\subset \gl_n$ 
consisting of upper triangular matrices is solvable. In some sense, this is the 
``only'' example over an algebraically closed field. 
\end{example}
...
filtration 
\begin{align*}
  G_1 &= \begin{pmatrix} 1 & \ast & \cdots & \ast \\ & 1 & \cdots & \vdots \\ & & \ddots & \ast \\ & & & 1 \end{pmatrix} \\
  \vdots \\
  G_{n-1} &= \begin{pmatrix} 1 & 0 & \cdots & \ast \\ & 1 & \ddots & \vdots \\ & & & 0 \\ & & & 1\end{pmatrix} \\
  G_n &= 1 .
\end{align*}
...
[Cite SGA for existence of $\derived G$.]

The basic idea is that $\derived G$ is the algebraic group generated by 
$\{x y x^{-1} y^{-1}:x,y\in G\}$. 

\begin{theorem}
Let $G_{/k}$ be a linear algebraic group. If $G$ is connected (resp.~smooth) 
then so is $\derived G$. 
\end{theorem}
...
It is always true that $(G/N)(\bar k) = G(\bar k)/N(\bar k)$. 

\begin{hard}
In general, we could look at the functor $A\mapsto G(A)/N(A)$. This will not 
be representable, but we can take its fppf sheafification, and hope that this 
is representable. [Cite the appropriate references in SGA]
\end{hard}
